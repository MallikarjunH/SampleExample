    ...
    [WebserviceManager sendSyncRequestWithURLGet:kGetAllSigners method:SAServiceReqestHTTPMethodGET body:kGetAllSigners completionBlock:^(BOOL status, id responseValue) {
        ...
        if(status && ![[responseValue valueForKey:@"Response"] isKindOfClass:[NSNull class]])
        {
            dispatch_async(dispatch_get_main_queue(),
                ...
                NSMutableDictionary *userTypeDict = [[NSMutableDictionary alloc]init];
                
                NSMutableArray * responseArray = [[NSMutableArray alloc]init];
                self->_ShowSignersList = [[NSMutableArray alloc]init];
                self->_holdSignersList = [NSMutableArray new];
                ...
                for (int i = 0; i<responseArray.count; i++) {
                    ...
                    if ([[responseArray[i]valueForKey:@"Email_Id"]isEqualToString:loggedInUserEmail]) {
                        //[userTypeDict setValue:@"2" forKey:@"UserType"];
                        userTypeDict = [responseArray[i] mutableCopy];
                       // [userTypeDict setValue:@"2" forKey:@"UserType"];
                        [userTypeDict setValue:@"ME" forKey:@"FullName"];
                        userTypeDict[@"UserType"] = @"2";
                       // [responseArray[i] addObject:userTypeDict];
                       // [self.holdSignersList insertObject:responseArray[i] atIndex:0];
                        [self.holdSignersList insertObject:userTypeDict atIndex:0];
                    }
                    else{
                       // [userTypeDict setValue:@"2" forKey:@"UserType"];
                         //userTypeDict = responseArray[i];
                        userTypeDict = [responseArray[i] mutableCopy];
                        // [userTypeDict setValue:@"2" forKey:@"UserType"];
                        userTypeDict[@"UserType"] = @"2";
                       // [responseArray[i] addObject:userTypeDict];
                      //  [self.holdSignersList addObject:responseArray[i]];
                        [self.holdSignersList addObject:userTypeDict];
                    }
                ...
                self->_ShowSignersList = [self.holdSignersList mutableCopy];
            ...
            dispatch_async(dispatch_get_main_queue(),
                ...
                for (int i = 0; i<responseArray.count; i++) {
                    ...
                    if ([[responseArray[i]valueForKey:@"Email_Id"]isEqualToString:loggedInUserEmail]) {
                       ...
                       // [userTypeDict setValue:@"4" forKey:@"UserType"];
                       // [responseArray[i] addObject:userTypeDict];
                       // [self.holdSignersList addObject:responseArray[i]];
                        userTypeDict = [responseArray[i] mutableCopy];
                        userTypeDict[@"UserType"] = @"4";
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
  ...
  //  NSArray *sortedArray = [self.ShowSignersList sortedArrayUsingDescriptors:sortDescriptors];
    ...
    NSMutableArray * signerArrList = [[NSMutableArray alloc]init];
    [signerArrList addObject:self.ShowSignersList[0]];
    
    [self.ShowSignersList removeObjectAtIndex:0];
     NSArray *sortedArray = [self.ShowSignersList sortedArrayUsingDescriptors:sortDescriptors];
    [signerArrList addObjectsFromArray:sortedArray];
    
    self.ShowSignersList = signerArrList;
    
    if(_searchResults == nil || _searchResults.count == 0){
        
       // section = [sortedArray objectAtIndex:indexPath.row];
        section = [signerArrList objectAtIndex:indexPath.row];
    }else{
        section = [self.searchResults objectAtIndex:indexPath.row];
    }
    

    if ([section valueForKey:@"FullName"] == (id)[NSNull null]) {
    ...
    if ([section valueForKey:@"Email_Id"] == (id)[NSNull null]){
    ...
    if ([[section valueForKey:@"UserType"]integerValue] == 4) {
        cell.userTypeImg.image = [UIImage imageNamed:@"adhocUser"];
    }else if ([[section valueForKey:@"UserType"]integerValue] == 2)
    {
        cell.userTypeImg.image = [UIImage imageNamed:@"user"];
    }else{
        cell.userTypeImg.image = [UIImage imageNamed:@""];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(_searchResults == nil || _searchResults.count == 0){
       ...
       // SubscriberId = [[sortedArray objectAtIndex:indexPath.row]valueForKey:@"SubscriberId"];
        SubscriberId = [[signerArrList objectAtIndex:indexPath.row]valueForKey:@"SubscriberId"];
        ...
        if (![sectionArray containsObject:[[signerArrList objectAtIndex:indexPath.row]valueForKey:@"FullName"]]) {
            
            //add data
            [self.holdNameandEmailDataForCollectionView setValue:[[signerArrList objectAtIndex:indexPath.row]valueForKey:@"FullName"] forKey:@"Name"];
            [self.holdNameandEmailDataForCollectionView setValue:[[signerArrList objectAtIndex:indexPath.row]valueForKey:@"Email_Id"] forKey:@"Email_Id"];
            
            if ([[signerArrList objectAtIndex:indexPath.row]valueForKey:@"FullName"] == (id)[NSNull null]) {
                ...
                [tableView headerViewForSection:secti].textLabel.text = [[signerArrList objectAtIndex:indexPath.row]valueForKey:@"FullName"];
                
                [sectionArray replaceObjectAtIndex:secti withObject:[[signerArrList objectAtIndex:indexPath.row]valueForKey:@"FullName"]];
            }
        ...
        if (!([[[signerArrList objectAtIndex:indexPath.section] description] rangeOfString:ReviewerToCompare].location == NSNotFound) && [[[signerArrList objectAtIndex:indexPath.row]valueForKey:@"FullName"]isEqualToString:@"ME"]) {
            
            [savedIndex replaceObjectAtIndex:indexPath.section withObject:@"Reviewer"];
            
        }else if(!([[[self.SignerssectionArray objectAtIndex:indexPath.section] description] rangeOfString:SignerToCompare].location == NSNotFound) && [[signerArrList objectAtIndex:indexPath.row]valueForKey:@"FullName"] == (id)[NSNull null]){
